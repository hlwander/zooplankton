...
##5Feb2020 --> fixed biomass calculation on line 202 (length was not cubed so biomass was too high!!)
#16Jun2020- taxa specific calcs were wrong becasue was only counting taxa with size measuremeents (changed aggregate Size_mm~Sample_ID to TaxaID~Sample_ID)
...
SizeID<-read.csv(dataFiles[2], fill = TRUE,as.is=TRUE)

#get rid of the first four practice samples
SizeID<- SizeID[-(1:78),]
Density<- Density[-(1:4),]
...
#Calculate the volume sampled represented by each subsample volume --> NOTE: only for 80 um net!!!
################### This is the 1-5mL in the counting chamber relative to    ########    This is the volume of the zoop tow          #####  This is conversion 
################### the concentrated total volume of the sample (100-1000mL) ########    depth of tow times area of circle (in m3)   #####   from m3 to L      
...
Density$Volume_unadj <- (Density$DepthOfTow_m*pi*RadiusOfZoopNet_m^2)   *    1000
...
### Note: For NA volume, subsample A was concentrated to 200 mL and B and C were concnetrated to 500 mL
...
taxaOfInterest<-c("Daphniidae","Copepoda","Calanoida","Cladocera","Cyclopoida","Rotifera","Keratella","Kellicottia","Crustacea","nauplius","Ceriodaphnia","Daphnia","Bosmina","Gastropus","Collotheca","Conochilus","Conochiloides","Synchaeta","Trichocerca","Holopedium","Lepadella","Monostyla","Lecane","Hexarthra","Polyarthra","Asplanchna","Ascomorpha")
CorrespondingTaxaLevel<-c("Family","Subclass","Order","Suborder","Order","Phylum","Genus","Genus","Subphylum","Nauplius","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus","Genus")
