...
zoop_totalcount <- zoop[(!is.na(zoop$mesh_size_μm) & zoop$mesh_size_μm==61 | zoop$sample_ID=="B_pel_12Aug20_noon_rep1" | zoop$sample_ID=="B_pel_12Aug20_noon_rep2" |
                           ...
                           zoop$sample_ID=="F10Sep20_9.0m"),keep]
...
for(i in 1:length(Density.neteff$sample_ID)){
  ...
  Density.neteff$Actual_dens[5:6] <- Schindler_totalCount_final$TotalCount_n[3] / (30*11)
...
NetEfficiency2020 <- c(mean(Density.neteff$NetEff[1:4]),mean(Density.neteff$NetEff[5:6]))
