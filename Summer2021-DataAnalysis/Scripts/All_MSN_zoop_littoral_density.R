...
#read in libraries
pacman::p_load(dplyr, vegan, labdsv, goeveg, rLakeAnalyzer, ggplot2,tidyr,lubridate, scales)
...
zoop_DHM<- zoop.repmeans[,substrEnd(colnames(zoop.repmeans),14)!="Total_rep.mean" & substrEnd(colnames(zoop.repmeans),12)!="Total_rep.SE"]

#convert new dfs fron tibble to dataframe 
zoop_DHM <- data.frame(zoop_DHM)

variables <- c("ZoopDensity_No.pL_rep.mean","Cladocera_density_NopL_rep.mean",
               "Cyclopoida_density_NopL_rep.mean","Rotifera_density_NopL_rep.mean",
               "Calanoida_density_NopL_rep.mean")
SE <- c("ZoopDensity_No.pL_rep.SE","Cladocera_density_NopL_rep.SE",
        "Cyclopoida_density_NopL_rep.SE","Rotifera_density_NopL_rep.SE",
        "Calanoida_density_NopL_rep.SE")

#only select density #/L cols
...
metric_taxa <-c("ZoopDensity","Cladocera","Cyclopoida",
                "Rotifera","Calanoida")
...
ggplot(subset(zoop_DHM_long, metric %in% c("Cladocera_density_NopL","Calanoida_density_NopL","Cyclopoida_density_NopL","Rotifera_density_NopL") &
                site_no %in% c("BVR_l")), aes(Hour,value, color=as.factor(MSN))) + 
  ...
  geom_point(size=2) + theme_bw() + facet_wrap(~metric,scales="free_y",labeller = labeller(metric=metric_taxa),ncol=5) + xlab("")+ coord_cartesian(clip = 'off') +
...
ggsave(file.path(getwd(),"Summer2021-DataAnalysis/Figures/BVR_MSNs_taxa_density_Littoral.jpg"), width=5, height=3) 
