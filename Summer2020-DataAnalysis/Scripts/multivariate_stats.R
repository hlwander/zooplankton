...
#also want to create a time_df to compare temporal zoop abundance 
#will need to select max, mean, and min temp, DO, etc... for each time (this could be an analysis for 2019 data because I think that's the onyl year we collected CTD casts every time we sampled)

#------------------------------------------------------------------------------#
#Nonmetric multidimensional scaling (NMDS) - trying first becasue Bryan sugegsted some sort of ordination
...
#------------------------------#
# run NMDS for spatial density #
#------------------------------#
par(ask=TRUE)
X <- metaMDS(zoop_dens_trans, distance='bray', k=3, trymax=20, autotransform=FALSE, pc=FALSE, plot=FALSE)
names(X)
X$stress
X$species
X$points
...
plot(X, display=c('sites','species'),choices=c(2,3), type='n')
points(X$points[depth_df$zone=='epi',1], X$points[depth_df$zone=='epi',2], pch=21,bg='black')
points(X$points[depth_df$zone=='hypo',1], X$points[depth_df$zone=='hypo',2], pch=21,bg='grey75')
#legend(min(X$points[,1]), max(X$points[,2]), legend=c('Night', 'Day'), pch=21, pt.bg=c('black', 'grey75')) 
legend("topleft", legend=c('epi','hypo'), pch=21, pt.bg=c('black', 'grey75'),bty = "n") 

ord <- ordiplot(X,display = c('sites'),choices = c(1,3),type = "points")
points(X$points[depth_df$zone=='epi',1], X$points[depth_df$zone=='epi',3], pch=21,bg='black')
points(X$points[depth_df$zone=='hypo',1], X$points[depth_df$zone=='hypo',3], pch=21,bg='grey75')
legend("topleft", legend=c('epi','hypo'), pch=21, pt.bg=c('black', 'grey75'),bty = "n") 
legend("bottomright",legend = c("k = 3"),cex = 1.4,bty = 'n')

ordihull(ord, depth_df$zone, display = "sites", draw = c("polygon"),
         col = c("grey74","black"), alpha = 75,cex = 2)

#------------------------------#
#run NMDS for temporal density #
#------------------------------#
par(ask=TRUE)
X <- metaMDS(zoop_temporal_dens_trans, distance='bray', k=3, trymax=20, autotransform=FALSE, pc=FALSE, plot=FALSE)
names(X)
X$stress
X$species
X$points
...
#  Points divided into day vs. night
plot(X, display=c('sites','species'),choices=c(2,3), type='n')
points(X$points[zoop_epi_tows$time=='sunrise',1], X$points[zoop_epi_tows$time=='sunrise',2], pch=21,bg='orange')
points(X$points[zoop_epi_tows$time=='noon',1], X$points[zoop_epi_tows$time=='noon',2], pch=21,bg='yellow')
points(X$points[zoop_epi_tows$time=='sunset',1], X$points[zoop_epi_tows$time=='sunset',2], pch=21,bg='pink')
points(X$points[zoop_epi_tows$time=='midnight',1], X$points[zoop_epi_tows$time=='midnight',2], pch=21,bg='darkblue')
legend("topleft", legend=c('sunrise','noon', 'sunset', 'midnight'), pch=21, pt.bg=c('orange', 'yellow', 'pink', 'darkblue'),bty = "n") 

ord <- ordiplot(X,display = c('sites'),choices = c(1,3),type = "points")
points(X$points[zoop_epi_tows$time=='sunrise',1], X$points[zoop_epi_tows$time=='sunrise',3], pch=21,bg='orange')
points(X$points[zoop_epi_tows$time=='noon',1], X$points[zoop_epi_tows$time=='noon',3], pch=21,bg='yellow')
points(X$points[zoop_epi_tows$time=='sunset',1], X$points[zoop_epi_tows$time=='sunset',3], pch=21,bg='pink')
points(X$points[zoop_epi_tows$time=='midnight',1], X$points[zoop_epi_tows$time=='midnight',3], pch=21,bg='darkblue')
legend("topleft", legend=c('sunrise','noon', 'sunset', 'midnight'), pch=21, pt.bg=c('orange', 'yellow','pink','darkblue'),bty = "n") 
legend("bottomright",legend = c("k = 3"),cex = 1.4,bty = 'n')

ordihull(ord, zoop_epi_tows$time, display = "sites", draw = c("polygon"),
         col = c("orange","yellow","pink","darkblue"), alpha = 75,cex = 2)
ordispider(ord, zoop_epi_tows$site_no, display="sites", w = weights(ord, display),
           spiders = c("centroid", "median"))
ordiellipse(X,zoop_epi_tows$site_no)
#super cool, but WHAT DOES IT MEAN?!?!

#  Basic outline for making your own Scree plot
#k <- c(1:6)
#Stress <- c(0.349,0.219,0.162,0.127,0.100,0.083)
#plot(k, Stress, type='b', xlab='Dimensions', ylab='Stress', pch=21, bg='grey75', cex=1.3)



#Maybe also try a PCA?
