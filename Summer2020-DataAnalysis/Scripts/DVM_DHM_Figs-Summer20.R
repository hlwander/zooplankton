...
ggplot(subset(zoop_DHM_long, metric %in% c("ZoopDensity_No.pL","BiomassConcentration_ugpL") & site_no %in% c("BVR_50","BVR_l") & (collect_date=="2020-08-12" | collect_date=="2020-08-13")), aes(Hour,value,color=site_no)) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-12 11:30:00"),xmax=as.POSIXct("2020-08-12 20:17:00"), ymin=-Inf, ymax= Inf, fill= "Noon"),color=NA) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-12 20:18:00"),xmax=as.POSIXct("2020-08-13 06:37:00"), ymin=-Inf, ymax= Inf, fill= "Midnight"),color=NA) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-13 06:38:00"),xmax=as.POSIXct("2020-08-13 12:30:00"), ymin=-Inf, ymax= Inf, fill= "Noon"),color=NA) +
...
ggsave(file.path(getwd(),"Figures/BVR_2020_aug_zoopdensity_biomass_LittoralvsPelagic.jpg")) 


#taxa density Aug 12-13
ggplot(subset(zoop_DHM_long, metric %in% c("Cladocera_density_NopL","Cyclopoida_density_NopL","Rotifera_density_NopL","Calanoida_density_NopL") &
                site_no %in% c("BVR_50","BVR_l") & (collect_date=="2020-08-12" | collect_date=="2020-08-13")), aes(Hour,value,color=site_no)) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-12 11:30:00"),xmax=as.POSIXct("2020-08-12 20:17:00"), ymin=-Inf, ymax= Inf, fill= "Noon"),color=NA) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-12 20:18:00"),xmax=as.POSIXct("2020-08-13 06:37:00"), ymin=-Inf, ymax= Inf, fill= "Midnight"),color=NA) +
  geom_rect(aes(xmin=as.POSIXct("2020-08-13 06:38:00"),xmax=as.POSIXct("2020-08-13 12:30:00"), ymin=-Inf, ymax= Inf, fill= "Noon"),color=NA) +
  geom_point(size=2) + theme_bw() + facet_grid(site_no~metric,scales="free_y",labeller = labeller(site_no=sites,metric=metric)) + xlab("")+ coord_cartesian(clip = 'off') +
  ...
  scale_fill_manual("",values=c("#CCCCCC","white"), guide = guide_legend(keyheight = .7,keywidth = 1.7, order=2, override.aes = list(alpha = 1,color="black")))+
  geom_errorbar(aes(ymin=value-value.SE, ymax=value+value.SE), width=.2,position=position_dodge(.9))
ggsave(file.path(getwd(),"Figures/BVR_2020_aug_taxa_density_LittoralvsPelagic.jpg")) 

#taxa biomass Aug 12-13
ggplot(subset(zoop_DHM_long, metric %in% c("Cladocera_BiomassConcentration_ugpL","Cyclopoida_BiomassConcentration_ugpL","Rotifera_BiomassConcentration_ugpL","Calanoida_BiomassConcentration_ugpL") &
  ...
  scale_fill_manual("",values=c("#CCCCCC","white"), guide = guide_legend(keyheight = .7,keywidth = 1.7, order=2, override.aes = list(alpha = 1,color="black")))+
  geom_errorbar(aes(ymin=value-value.SE, ymax=value+value.SE), width=.2,position=position_dodge(.9))
ggsave(file.path(getwd(),"Figures/BVR_2020_aug_taxa_biomass_LittoralvsPelagic.jpg"))

#------------------------------------------------------------------------------------#
##Figs - more taxa specific density over 24 hrs
...
ggplot(subset(zoop_DHM_bytaxa_long, grepl("density",metric,ignore.case = TRUE) &
  ...
  geom_errorbar(aes(ymin=value-value.SE, ymax=value+value.SE), width=.2,position=position_dodge(.9))
ggsave(file.path(getwd(),"Figures/BVR_2020_aug_taxa2_density_LittoralvsPelagic.jpg")) 
...
ggplot(subset(zoop_DHM_bytaxa_long, grepl("biomass",metric,ignore.case = TRUE) &
  ...
  scale_fill_manual("",values=c("#CCCCCC","white"), guide = guide_legend(keyheight = .5,keywidth = .5, order=2, override.aes = list(alpha = 1,color="black")))+
  geom_errorbar(aes(ymin=value-value.SE, ymax=value+value.SE), width=.2,position=position_dodge(.9))
ggsave(file.path(getwd(),"Figures/BVR_2020_aug_taxa2_biomass_LittoralvsPelagic.jpg"))
...
#replace NAN with 0
BVR.DVM.percent.dens.calcs$value <- ifelse(is.na(BVR.DVM.percent.dens.calcs$value), 0, BVR.DVM.percent.dens.calcs$value)
